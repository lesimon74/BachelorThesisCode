...
for aa in range(0, numAA):
    ...
    for frame in universe.trajectory:
        ...
        if frameNr == 0:
            ...
            # make cumulative
            for y in range(1, 50):
                prevBins[y] = prevBins[y] + prevBins[y-1]
        else:
            for aa2 in range(0, numAA):
                cartesians2 = universe.residues[aa2].atoms.center_of_mass()
                StoreDistances[aa2] = np.sqrt(np.sum(np.square(cartesians-cartesians2)))
            ...
            # procedure:
            # a) create histograms (prevBins and currentBins)
            # b) Calculate difference, take its absolute value and then sum it up.
            for x in range(1, 50):
                currentBins[x] = currentBins[x-1] + currentBins[x]
            avgDist[frameNr] = np.sum(np.abs(currentBins-prevBins))
            ###
